...
Given multiple metric time-series, detects anomalies via Z-Score and finds
temporally correlated anomaly pairs across different metrics.

Usage:
    python3 correlate.py --data-file /tmp/metric_data.json [--threshold 2.0] [--window 300]
  ...
  "metrics": [
    {
      ...
      "labels": {"service": "people-api", "quantile": "0.99"},
      "values": [[1722950400, 0.25], [1722950415, 0.27], ...]
    }
  ]
}
"""
...
def find_correlations(all_anomalies, window=300):
    """Find temporally correlated anomaly pairs across metrics."""
    correlations = []
    ...
    for c in sorted(
        correlations,
        key=lambda x: abs(x["zscore_a"]) + abs(x["zscore_b"]),
        reverse=True,
    ):
...
def main():
    parser = argparse.ArgumentParser(
        description="Metric correlation analysis via Z-Score anomaly detection"
    )
    ...
    parser.add_argument("--window", type=int, default=300,
                        help="Max seconds between correlated anomalies (default: 300)")
    ...
    report = {
        ...
        "conclusion": {
            "correlated_pairs": len(correlations),
            "strongest": correlations[0] if correlations else None,
            "summary": (
                f"{len(correlations)} pairs with co-occurring anomalies "
                f"within {args.window}s."
