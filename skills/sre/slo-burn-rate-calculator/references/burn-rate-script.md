# SLO Burn Rate Calculator Script
...
Usage:
    python3 /tmp/burn_rate.py --data-file /tmp/slo_data.json [--budget-days 30] [--target 0.999]
...
def calculate_burn_rate(errors, total, slo_target, budget_period_days, window_hours):
    ...
    if error_budget == 0:
        return {"burn_rate": float('inf'), "error_rate": error_rate, "status": "impossible_target"}
...
def main():
    ...
    worst_status = "ok"
    worst_window = None
    severity_order = ["ok", "WATCH", "SLOW_BURN_TICKET", "WARNING", "WARNING_TICKET", "CRITICAL_PAGE"]
    ...
    # Budget math
    error_budget_total = (1 - slo_target) * budget_days * 24 * 60  # in error-minutes
    longest_window = max(windows.keys(), key=lambda w: float(w.replace("h","").replace("d","")) * (24 if "d" in w else 1))
    longest_data = windows[longest_window]
    current_error_rate = longest_data["errors"] / longest_data["total"] if longest_data["total"] > 0 else 0
